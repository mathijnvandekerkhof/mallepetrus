    ...
    this.client.interceptors.response.use(
      ...
      (error) => {
        ...
        if (apiError.status === 401) {
          // Unauthorized - clear token and redirect to login (but not for setup endpoints)
          const isSetupEndpoint = error.config?.url?.includes('/setup/')
          if (!isSetupEndpoint) {
          ...
          // Don't show error messages for setup status checks
          const isSetupStatusCheck = error.config?.url?.includes('/setup/status')
          if (!isSetupStatusCheck) {
